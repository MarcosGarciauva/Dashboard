...
export async function ensureSession(session = getStoredSession()) {
  if (!session?.access_token) return null;
...
function fromSettings(row = {}) {
  return {
    ...
    pomodoroSound: row.pomodoro_sound === undefined || row.pomodoro_sound === null ? true : Boolean(row.pomodoro_sound),
